...
export default function MyWallet(props: { wallet_id: string }) {
  ...
  const { data: walletAssetUpdated } = useSWRSubscription(`http://localhost:3000/wallets/${props.wallet_id}/assets/events`, (path, { next }: SWRSubscriptionOptions) => {
    const eventSource = new EventSource(path);
      ...
      <TableBody className='divide-y'>
        {walletAssets!.map((walletAsset, key) => (
          <TableRow className='border-gray-700 bg-gray-800' key={key}>
            ...
            <TableCell>
              <Link className='font-medium hover:underline text-cyan-500' href={`/${props.wallet_id}/home-broker/${walletAsset.Asset.id}`}>
                Comprar/Vender
              </Link>
            </TableCell>
          </TableRow>
